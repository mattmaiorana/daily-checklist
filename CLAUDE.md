...
- **Show edit link in sidebar** — toggle. **Default: `true`.** Storage key `showSidebarEditLink`. When off, the `(edit)` control next to the sidebar title is not rendered, and the section renders only in normal (non-edit) mode. Toggling the setting calls `refreshViews()`, so a user who was mid-edit when they hid the link returns to the calm non-edit view rather than being stuck in invisible edit mode. Edit-mode internals are unchanged — checklist items can still be added / renamed / deleted / reordered from the settings tab's checklist editor. No daily-note write occurs from toggling this setting.
